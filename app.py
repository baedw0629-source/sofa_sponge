...
def load_data():
    try:
        # utf-8-sig로 한글 깨짐 방지 및 강력한 공백 제거 처리
        df = pd.read_csv('spongematerials.csv', encoding='utf-8-sig')
        df.columns = df.columns.str.strip().str.replace(' ', '')
        
        # 단가 숫자 변환 (콤마, 원 제거 및 숫자 강제 변환)
...
def calculate_row(row):
    ...
    ah_mat_cost = af_qty * u_price if calc_mode == "발포업체" else af_qty * (1.0 + float(loss_rate_val)) * u_price
...
if st.button("🚀 최종 단가 산출하기"):
    if not edited_df.empty:
        ...
        # CSV 파일 변환 (엑셀에서 바로 열 수 있도록 utf-8-sig 적용)
        csv_data = final_df.to_csv(index=True, index_label="No").encode('utf-8-sig')
        
        st.download_button(
            label="📥 결과 저장(CSV)",
            data=csv_data,
            file_name="sponge_calc_result.csv",
            mime="text/csv"
        )
    else:
