...
if "input_df" not in st.session_state:
    # 1번 요청: 초기값을 0으로 비워진 상태로 시작
    st.session_state.input_df = pd.DataFrame([{
        "선택업체": "진양", "재질": "선택하세요", "재단방식": "일반",
        "W(사선)": 0.0, "W": 0.0, "D": 0.0, "T": 0.0
    }])
...
# 2번 요청: 행 추가 시 '진양' 고정을 위해 num_rows="dynamic" 사용
edited_df = st.data_editor(
    st.session_state.input_df,
    num_rows="dynamic",
    column_config={
        "선택업체": st.column_config.SelectboxColumn("선택업체", options=["진양", "폼웍스"], required=True),
...
def calculate_row(row):
    if row['재질'] == "선택하세요": return pd.Series(["-", "-", 0, 0, 0, 0], index=["밀도", "경도", "소요량(평)", "재료비", "가공비", "최종단가"])
    vol_unit = 303 * 303 * 10
...
if st.button("🚀 최종 단가 산출하기"):
    ...
    st.session_state.last_result = final_df # 결과 저장
