...
with tab1:
    ...
    edited_df = st.data_editor(
        ...
        num_rows="dynamic",
        column_config={
            # [요청 반영] 행 추가 시 진양 고정 및 선택창 팝업 억제
    ...
    def calculate_row(row):
        ...
        # 업체 성격 정의 (사용자 엑셀 수식 기준)
        is_foam_company = (row['선택업체'] == "진양") # 엑셀 5510원 결과를 맞추기 위해 진양을 발포업체로 가정
        u_price = float(mat_info['가공업체단가' if is_foam_company else '발포업체단가'].values[0])
        ...
        ah_mat = excel_round(excel_round(af_qty_internal * u_price, 0) * (1.0 + (loss_rate if is_foam_company else 0)), 0)
        
        # 2. 가공비
        calc_proc = 0.0
        if row['재단방식'] == "2D": calc_proc = ah_mat * 0.2
        elif is_foam_company:
            if row['재단방식'] == "일반": calc_proc = (w/1000 * d/1000 * h_cut) + (w/1000 * d/1000 * t * v_cut)
            elif row['재단방식'] == "사선": calc_proc = ((ws + w)/1000 * d/1000 * v_cut * t) + (w/1000 * d/1000 * h_cut)
        ai_proc = excel_round(calc_proc, 0)

        # 3. 경비, 관리비, 이윤 (이윤 57원 동기화)
    ...
    if st.session_state.last_result is not None:
        ...
        # [요청 반영] 버튼 및 입력 칸 동일 높이 배치
        st.write("") 
        col_name, col_hist, col_csv = st.columns([3, 1, 1])
        with col_name:
            hist_name = st.text_input("히스토리 명칭", value=datetime.now().strftime("%m%d_%H%M"), label_visibility="collapsed", placeholder="히스토리 명칭 입력")
        with col_hist:
